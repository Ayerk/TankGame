...
public class LevelButton extends ElementObj{
	
	private int level; //表示该按钮表示第几关
	...
	public ElementObj createElement(String str) {
		...
		int margin_x = 30;
		int margin_y = 30;
		this.setLevel(Integer.parseInt(str));
		int column= this.getLevel()>5?this.getLevel()-5:this.getLevel();
		int row = this.getLevel()>5?1:0;
		
		this.setX(init_x+(column-1)*(margin_x + this.getW()));
		this.setY(init_y+row*(margin_y+this.getH()));
		return this;
	}
